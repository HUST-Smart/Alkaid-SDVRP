...
inline void
SdSwapStar(const Problem &problem, const DistanceMatrix &distance_matrix,
           const Solution &solution, const RouteContext &context, bool swapped,
           Node route_x, Node route_y, Node node_x, Node node_y, int split_load,
           const std::vector<BestInsertion<3>> &insertions,
           SdSwapStarMove &best_move, Delta<int> &best_delta, Random &random) {
  auto &&insertion_x = insertions[node_x];
  auto &&insertion_y = insertions[node_y];
...
void SdSwapStar(const Problem &problem, const DistanceMatrix &distance_matrix,
                ...
                std::vector<BestInsertion<3>> &insertions, Random &random) {
  PreprocessInsertions(distance_matrix, solution, context, route_x, route_y,
                       insertions, random);
  PreprocessInsertions(distance_matrix, solution, context, route_y, route_x,
                       insertions, random);
...
bool SdSwapStar(const Problem &problem, const DistanceMatrix &distance_matrix,
  ...
  static std::vector<BestInsertion<3>> insertions;
  SdSwapStarMove best_move{};
  Delta<int> best_delta;
  Node max_index = *std::max_element(
      solution.node_pool(), solution.node_pool() + solution.num_nodes());
  insertions.clear();
  insertions.resize(max_index + 1);
    ...
    for (Node route_y = route_x + 1; route_y < context.num_routes; ++route_y) {
      ...
      if (!cache.TryReuse()) {
        SdSwapStar(problem, distance_matrix, solution, context, route_x,
                   route_y, move, cache.delta, insertions, random);
      } else {
        if (!move.swapped) {
