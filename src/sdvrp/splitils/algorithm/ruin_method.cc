...
std::vector<Node> RandomRuin::Ruin(const Problem &problem,
...
SisrsRuin::SisrsRuin(int average_customers, int max_length, double split_rate,
    ...
    : average_customers_(average_customers), max_length_(max_length),
      split_rate_(split_rate), preserved_probability_(preserved_probability) {}

std::vector<Node> SisrsRuin::Ruin(const Problem &problem,
                                  const DistanceMatrix &distance_matrix,
                                  Solution &solution, RouteContext &context,
                                  Random &random) {
  double average_length =
      static_cast<double>(problem.num_customers - 1) / context.num_routes;
  double max_length =
      std::min(static_cast<double>(max_length_), average_length);
  double max_strings = 4.0 * average_customers_ / (1 + max_length_) - 1;
  int num_strings = static_cast<int>(random.NextFloat() * max_strings) + 1;
  int customer_seed = random.NextInt(1, problem.num_customers - 1);
  std::vector<Node> node_indices(solution.node_pool(),
                                 solution.node_pool() + solution.num_nodes());
  auto &&seed_distances = distance_matrix[customer_seed];
  std::stable_sort(node_indices.begin(), node_indices.end(),
                   [&](Node lhs, Node rhs) {
                     return seed_distances[solution.customer(lhs)] <
                            seed_distances[solution.customer(rhs)];
                   });
  ...
  for (Node node_index : node_indices) {
    ...
    double max_ruin_length =
        std::min(static_cast<double>(route_length), max_length);
    int ruin_length =
        static_cast<int>(random.NextFloat() * max_ruin_length) + 1;
    int num_preserved = 0;
    int preserved_start_position = -1;
    if (ruin_length >= 2 && ruin_length < route_length &&
        random.NextFloat() < split_rate_) {
      ...
      preserved_start_position =
          random.NextInt(1, ruin_length - num_preserved - 1);
    }
    ...
    for (int j = 0; j < ruin_length; ++j) {
      if (j < preserved_start_position ||
          j >= preserved_start_position + num_preserved) {
        customer_indices.emplace_back(
            solution.customer(route[start_position + j]));
      }
    }
  }
  std::sort(customer_indices.begin(), customer_indices.end());
  customer_indices.erase(
      std::unique(customer_indices.begin(), customer_indices.end()),
      customer_indices.end());
  Shuffle(customer_indices.begin(), customer_indices.end(), random);
  return customer_indices;
}

Node SisrsRuin::GetRouteHead(Solution &solution, Node node_index,
                             int &position) {
  position = 0;
  while (true) {
...
void SisrsRuin::GetRoute(const Solution &solution, Node head,
                         std::vector<Node> &route) {
  route.clear();
  while (head) {
